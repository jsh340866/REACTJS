...
export function Grade({ score }) {
  ...
  return ( <p>점수 {score}점 - 등급 {score>=90?"A": score>=80?"B": score>=70?"C" : "F" } {score===100 && <b>만점!</b>} </p>) ;
}
...
export function StatusBadge({ status }) {
  ...
  return ( <p>주문 상태 : {((status)=>{ if(status === "ready") return "준비중"; if(status === "shipping") return "배송중"; else return "배송완료";})(status)}</p>);
}
...
export function UserName({ name, isAdmin }) {
  ...
  return ( <p>{name || "익명 사용자"} {isAdmin && "(관리자)"}</p>);
}
...
export function SecretBox({ isOpen }) {
  ...
  else return (<div>비밀 내용이 공개되엇습니다.</div>)
}
